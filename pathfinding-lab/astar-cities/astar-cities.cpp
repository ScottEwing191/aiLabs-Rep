...
public:
    ...
    CostType operator()(Vertex u)
    {
        ...
        CostType dy = m_location[m_goal].y - m_location[u].y;
        //return ::sqrt(dx * dx + dy * dy);
        return (dx * dx + dy * dy);                                 // Modify the A* heuristic involved
...
int main(int argc, char **argv)
{
  ...
  location locations[] = { // lat/long                      // a.Ithica lat,long = {42.44, 76.50}
  ...
  edge edge_array[] = {
    ...
    /*edge(Binghamton,Woodstock),*/ edge(Binghamton,NewYork),       //Woodstock removed
    edge(Syracuse,Binghamton)/*, edge(Woodstock,Troy),
    edge(Woodstock,NewYork)*/
  };
  unsigned int num_edges = sizeof(edge_array) / sizeof(edge);       // b.Ithica and Binghampton weight = 70
  cost weights[] = { // estimated travel time (mins)
    96, 134, 143, 65, 115, 133, 117, 116, 74, 56,
    84, 73, 69, 70, 116, 147, /*173,*/ 183, 74/*, 71, 124*/         //Edge weights involving woodstock removed
  };
  ...
  std::cout << "Pick a start city: Troy, Lake Placid, Plattsburgh, Massena,Watertown, Utica, Syracuse, Rochester, Buffalo, Ithaca, Binghamton, Woodstock, New York" << std::endl;
